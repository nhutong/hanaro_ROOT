...
$("#delivery_submit_btn").on("click",function(){
	...
	$.ajax({
        url:'/back/02_app/mLeafletDeliveryInsert.jsp?random=' + (Math.random()*99999),
		data : {
        ...
        if(result.trim() == 'NoN' || result == 'list error' || result == 'empty'){
            ...
            data['PdContentList'].forEach(function(item, index){    
				...
				if ( item['jang_cnt'] == 0 )
				{
					...
					text += '			<a href="#" class="price">'+comma(item['price'])+'<h6 style="font-family: Noto Sans KR;display:inline-block;font-weight: 500;">원</h6></a>' // 20200619 김수경 원 살림
					text += '	</div>';
					text += '</div>';
                    text += '</div>';
...
/* 배송가능지역 select 20191229*/
    ...
    $.ajax({
        url:'/back/02_app/mLeafletDelivery.jsp?random=' + (Math.random()*99999), 
        data : {vm_cp_no: vm_cp_no},
        method : 'GET' 
    }).done(function(result){
...
/* 배송회차 select 20191229*/
    ...
    $.ajax({
        url:'/back/02_app/mLeafletDeliveryRound.jsp?random=' + (Math.random()*99999), 
        data : {vm_cp_no: vm_cp_no},
        method : 'GET' 
    }).done(function(result){
