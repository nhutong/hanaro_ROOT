...
function qnaCont(rcvNtNo){
		...
		$.ajax({
			url:'https://www.nhhanaromart.com/back/04_home/qnaDetail.jsp?random=' + (Math.random()*99999), 
			data : {memberNo: localStorage.getItem("memberNo") , vm_cp_no: localStorage.getItem("vm_cp_no"), nt_no: rcvNtNo },
			method : 'GET' 
...
function qnaContRe(rcvNtNo){
		...
		$.ajax({
			url:'https://www.nhhanaromart.com/back/04_home/qnaRe.jsp?random=' + (Math.random()*99999), 
			...
			data['CompanyList'].forEach(function(item, index){    
				...
				text +='   <div class="question_cont_wrap">';
				/* 20200226 추가 시작 */
				text +='	   <p>'+item['nt_content'].replace(re,"<br>")+'</p>';
				/* 20200226 추가 끝 */
				text +='   </div>';
				text +='</div>';
			});
