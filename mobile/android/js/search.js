...
function getPdContent() {
    ...
    $.ajax({
        url:'https://www.nhhanaromart.com/back/02_app/mLeafletSearch.jsp?random=' + (Math.random()*99999), 
        ...
        if(result.trim() == 'NoN' || result == 'list error' || result == 'empty'){
            ...
            data['PdContentList'].forEach(function(item, index){    
                ...
                text += '		<a href="#"><img src="https://www.nhhanaromart.com'+item['img_path']+'" alt="'+item['pd_name']+'"></a>'
                text += '		<div class="thumb_info">'

				/* 로그인 & 장바구니에 이 물건을 담지 않은 사용자만 상품별 장바구니 추가버튼이 보인다. 20191229*/
				if (localStorage.getItem("memberNo") != "" && item['vmjz_no'] == "")
				{
