...
//console.log(jsonResult.CompanyList);
		...
		for(var i in jsonResult_comp){
			...
			text +='	<div class="locationClick" style="width:250px;" onclick="convertToMain('+jsonResult_comp[i].VM_CP_NO+', \''+jsonResult_comp[i].VM_CP_NAME+'\', \''+jsonResult_comp[i].VM_delivery_FG+'\');">';
...
//매장페이지로 이동하기
function convertToMain(rcvCPNO, cp_name, delivery_fg){

	var result = confirm(""+cp_name+"을 선택하시겠습니까?");
	if(result){

			localStorage.setItem("vm_cp_no",rcvCPNO);
			//localStorage.setItem("VM_delivery_FG",delivery_fg);
			...
			alert("하나로마트 "+cp_name+"을 선택하셨습니다. 이용매장은 앱 내 메뉴(≡)에서 변경 가능합니다.");
