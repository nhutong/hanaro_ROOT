...
function joinEvent(rcvEventNo){
	...
	$.ajax({
		url:'https://www.nhhanaromart.com/back/02_app/mEventJoin.jsp?random=' + (Math.random()*99999), 
...
function eventList(rcv_vm_cp_no){
		...
		$.ajax({
			url:'https://www.nhhanaromart.com/back/02_app/mEvent.jsp?random=' + (Math.random()*99999), 
...
//						text +='	  <button onclick="location.href=\'file:///android_asset/www/'+jsonResult_notice[i].link_url+'\';">자세히 보기</button>';   
//                    }

					if (jsonResult_notice[i].eventLink == "N")
                    {
                        text +='	   <img src="https://www.nhhanaromart.com'+jsonResult_notice[i].detail_img_url+'" alt="이미지">';
                        text +='	</div>';
                        //text +='	  <p></p>'; 
//                        text +='	  <button onclick="joinEvent('+jsonResult_notice[i].event_no+');">참여하기</button>';                          
                    }else{
...
//                        text +='	  <button onclick="window.open(\''+jsonResult_notice[i].link_url+'\');">자세히 보기</button>';  
						...
						{/* 20200229 하기 1줄 수정 보러가기 */
							text +='	  <button onclick="location.href=\'file:///android_asset/www/'+jsonResult_notice[i].link_url+'\';">보러가기</button>'; 
						}else{
...
function eventListIng(rcv_vm_cp_no){
		...
		$.ajax({
			url:'https://www.nhhanaromart.com/back/02_app/mEventIng.jsp?random=' + (Math.random()*99999), 
			data : {userCompanyNo: rcv_vm_cp_no},
			method : 'GET' 
		}).done(function(result){
...
function eventListEnd(rcv_vm_cp_no){
		...
		$.ajax({
			url:'https://www.nhhanaromart.com/back/02_app/mEventEnd.jsp?random=' + (Math.random()*99999), 
			data : {userCompanyNo: rcv_vm_cp_no},
			method : 'GET' 
		}).done(function(result){
...
//					text +='	</div>';
//					//text +='	  <p></p>'; 
////					text +='	  <button class="end" onclick="joinEvent('+jsonResult_notice[i].event_no+');">종료된 이벤트입니다.</button>'; 
//					text +='	  <button class="end">종료된 이벤트입니다.</button>'; 
