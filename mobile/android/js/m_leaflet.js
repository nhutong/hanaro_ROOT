...
// rcv_next_fg: (-1)rcv_jd_no 이전전단, (xx)해당 전단 / 0이면 오늘자 전단, (1)rcv_jd_no 다음전단
function getJd(rcv_vm_cp_no, rcv_menu_no, rcv_jd_no, rcv_interval){
    ...
    $.ajax({
        url:'https://www.nhhanaromart.com/back/02_app/mLeafletGetJd.jsp?random=' + (Math.random()*99999), 
...
//					$(".bxslider").append('<li><img src="'+decodeURIComponent(item['jb_img_path']).replace(/\+/g,' ')+'" style="width : 100%;"></li>');
					$(".bxslider").append('<li><img src="https://www.nhhanaromart.com'+decodeURIComponent(item['jb_img_path']).replace(/\+/g,' ')+'" style="width : 100%;"></li>');
				}					
			});
