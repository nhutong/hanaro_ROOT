...
<%
	...
	String pushStatus = request.getParameter("pushStatus") == null ? "" : request.getParameter("pushStatus").trim();		
	...
	try{
		...
		sql = " insert into vm_push_message (ms_content, vm_cp_no, event_no, reg_no, reg_date, pm_hour, pm_min, pm_img_path, pm_from_date, pm_to_date, pm_interval, pm_target, pm_type, del_fg, pm_status) "
			+ " values('[광고] "+pushTopTxt+" 수신거부 | 메뉴>설정>동의 해제', '"+vm_cp_no+"', '"+event_no+"', '"+reg_no+"', now(), '"+pushSendHr+"', '"+pushSendMin+"', '"+pm_img_path+"', if('"+pushSendFromDate+"' = '',null,'"+pushSendFromDate+"'), if('"+pushSendToDate+"' = '',null,'"+pushSendToDate+"'), '"+pushInterval+"', '"+pushTarget+"', '"+pushType+"', '"+pushDel+"', '"+pushStatus+"'); ";
        //out.print(sql);
