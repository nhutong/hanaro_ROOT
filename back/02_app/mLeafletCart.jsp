...
<%	
	...
	try{
		...
		sql = " SELECT b.jd_prod_con_no, ifnull(d.img_path,'') as img_path, ifnull(ak.VM_delivery_FG,'N') as VM_delivery_FG, "
			...
			+" ifnull(b.coupon_discount,'') as coupon_discount, ifnull(b.dadaiksun,'') as dadaiksun, ifnull(b.dadaiksun_info,'') as dadaiksun_info, ifnull(b.etc,'') as etc, c.pd_no, c.pd_code, ifnull(e.vmjz_no,'') as vmjz_no, jang_cnt "
...
//	out.print(sql);
		...
		while(rs.next()){
			...
			String jang_cnt				   = rs.getString("jang_cnt");
			...
			obj.put("jang_cnt", strDecode(jang_cnt));
