...
//	페이징 - 총 페이징 사이즈 ( 페이징 리스트에 보여줄 페이징 숫자의 갯수 )
	...
	try{
		...
		sql = "select count(a.cont_no) as pd_cnt "
		    + " from eb_page_contact_log AS a inner join vm_user as b on a.session_id_name = b.vm_no WHERE LEFT(a.LST_UPDATE_DATE,10) >= date_add(now(),INTERVAL -14 DAY) "; 

		if (searchText != ""){
           sql = sql + " and ( p.title like '%"+searchText+"%' or p.content like '%"+searchText+"%' )";
		}else{
		   sql = sql + " ";
		}
