...
%><%@ include file = "auth.jsp" %><%-- 권한체크 --%><%
  ...
  // 파라미터
	...
	String postStatusCd = "DELETED"; // 게시물 상태코드 : 삭제(DELETED)
 	...
 	try {
		...
		Map<String, Object> previous =  
			queryRunner.query(
				...
				" WHERE p.post_no = ? AND p.ref_post_no = ? AND p.ref_cp_no = ? AND p.post_type_cd = ? ",
		...
		results.put("delete", 
			queryRunner.update(
				...
				" WHERE post_no = ? AND ref_post_no = ? AND ref_cp_no = ? AND post_type_cd = ? ",
